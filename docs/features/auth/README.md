...
Приложение поддерживает безопасный сброс пароля через email с использованием токенов с ограниченным сроком действия.

**Поток работы:**

1. Пользователь запрашивает сброс пароля (`/forgot-password`)
2. Система генерирует случайный токен и отправляет его на email
3. Токен хранится в БД в захешированном виде (SHA-256)
4. Пользователь переходит по ссылке из письма (`/reset-password/[token]`)
5. Система проверяет токен и срок его действия
6. При успешной проверке пользователь устанавливает новый пароль
7. Токен удаляется из БД

### Структура БД (миграция 0004)
...
- `password_reset_token_hash` - SHA-256 хеш токена (hex-формат, 64 символа)
- `password_reset_expires_at` - время истечения токена в ISO-8601 формате
...
{
	"success": true,
	"message": "Если email зарегистрирован, мы отправили инструкции для сброса пароля"
}
```
...
- Одинаковый ответ для существующих/несуществующих email (предотвращение enumeration)
...
- Токен существует и не истёк
- Пароль соответствует требованиям (минимум 8 символов, буквы + цифры)
- Пароли совпадают
...
- Генерируются криптографически стойким способом (`crypto.getRandomValues()`)
- Хранятся в БД в виде SHA-256 хеша
- Срок действия: 1 час
- Одноразовые (удаляются после использования)

**Email защита:**

- Не раскрывается информация о существовании email в системе
- Отправка происходит асинхронно (не блокирует ответ API)
- DKIM подпись для защиты от спуфинга

**Rate Limiting:**

- Cloudflare Workers автоматически ограничивает количество запросов
- Turnstile защита на форме запроса сброса
...
- **API Routes:**
  - `src/routes/api/auth/register/+server.ts` - Регистрация пользователя
  - `src/routes/api/auth/login/+server.ts` - Вход в систему
  - `src/routes/api/auth/logout/+server.ts` - Выход из системы
  - `src/routes/api/auth/forgot-password/+server.ts` - Запрос на сброс пароля
  - `src/routes/api/auth/reset-password/+server.ts` - Сброс пароля по токену
- **UI Routes:**
  - `src/routes/login/+page.svelte` - Страница входа
  - `src/routes/register/+page.svelte` - Страница регистрации
  - `src/routes/forgot-password/+page.svelte` - Страница запроса сброса пароля
  - `src/routes/reset-password/[token]/+page.svelte` - Страница сброса пароля
