...
Модуль email отвечает за отправку уведомлений пользователям через различные email провайдеры.
...
**💡 Рекомендация:** Используйте Resend, если домен `kolibri-dresden.de` не находится в зоне Cloudflare (например, DNS на IONOS), так как MailChannels требует домен в том же Cloudflare аккаунте.
...
**Для Resend провайдера:**

```bash
EMAIL_PROVIDER=resend
RESEND_API_KEY=re_xxxxxxxxxxxxxxxxxxxxxxxxxx  # API ключ из Resend Dashboard
```

**💡 Рекомендация:** Используйте Resend, если домен не находится в зоне Cloudflare. Для MailChannels домен должен быть добавлен как site в том же Cloudflare аккаунте.
...
**DKIM (опционально для MailChannels, управляется Resend автоматически):**
