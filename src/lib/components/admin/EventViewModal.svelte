<script lang="ts">
	...
	// Состояние для управления постером
	let uploading = false;
	let posterError = '';
	...
	async function handleFileSelect(event: { currentTarget: HTMLInputElement }) {
		...
		if (!allowedTypes.includes(file.type)) {
			posterError = $_('admin.events.poster.invalidType');
			input.value = '';
			return;
		}
		...
		if (file.size > maxSize) {
			posterError = $_('admin.events.poster.tooLarge');
			input.value = '';
			return;
		}
	...
	async function uploadPoster(file: File) {
		...
		try {
			...
			const response = await fetch(`/api/admin/events/${eventId}/poster`, {
				method: 'POST',
				body: formData,
			});
			...
			if (!response.ok) {
				const data = await response.json();
				throw new Error(data.error || $_('admin.events.poster.uploadFailed'));
			}

			const data = await response.json();

			// Обновляем URL постера в локальном состоянии
			if (event) {
				event.poster_url = data.poster_url;
			}
		} catch (err: any) {
			console.error('Poster upload failed:', err);
			posterError = err.message || $_('admin.events.poster.uploadFailed');
		} finally {
			uploading = false;
		}
	}
	...
	async function handleRemovePoster() {
		if (!confirm($_('admin.events.poster.removeConfirm'))) {
			return;
		}
		...
		try {
			...
			if (!response.ok) {
				const data = await response.json();
				throw new Error(data.error || $_('admin.events.poster.removeFailed'));
			}
			...
			posterError = err.message || $_('admin.events.poster.removeFailed');
		} finally {
			uploading = false;
		}
	}
	...
	<Modal title={$_('admin.events.viewEventTitle')} on:close={handleClose} size="lg">
		<div class="space-y-6">
			...
			<div class="border border-gray-200 rounded-md p-4">
				<h3 class="text-lg font-semibold text-gray-900 mb-2">
					{$_('admin.events.poster.title')}
				</h3>
				<p class="text-sm text-gray-600 mb-3">
					{$_('admin.events.poster.help')}
				</p>
				...
				{#if event.poster_url}
					<div class="mb-4">
						<div
							class="relative w-full max-w-md mx-auto bg-gray-50 border border-gray-300 rounded-md overflow-hidden"
							style="max-height: 300px;"
						>
							<img
								src={event.poster_url}
								alt={$_('admin.events.poster.title')}
				...
				{#if posterError}
					<div
						class="mb-3 bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-md text-sm"
					>
						{posterError}
					</div>
				{/if}
					...
					<label class="flex-1 min-w-[200px]">
						...
						>
							{#if uploading}
								...
								{$_('admin.events.poster.uploading')}
							{:else}
								<svg
									class="w-4 h-4 mr-2"
									fill="none"
								...
								{event.poster_url
									? $_('admin.events.poster.replace')
									: $_('admin.events.poster.upload')}
							{/if}
						</Button>
					</label>

					<!-- Кнопка удаления (только если постер есть) -->
					{#if event.poster_url}
						<Button
							on:click={handleRemovePoster}
							variant="secondary"
							disabled={uploading}
							class="min-w-40"
						>
