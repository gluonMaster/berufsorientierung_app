<script lang="ts">
	...
	export let registrationsCount: number = 0;
	export let onClose: () => void = () => {};
	export let onCancel: (reason: string) => Promise<void> = async () => {};
	...
	async function handleSubmit() {
		...
		if (cancellationReason.trim().length < 10) {
			error = $_('admin.events.errors.reasonTooShort');
			return;
		}
				...
				<div class="flex">
					...
					<div class="ml-3">
						...
						<div class="mt-2 text-sm text-orange-700">
							<p>
								{$_('admin.events.cancelWarningMessage', {
									values: { count: registrationsCount },
								})}
							</p>
						</div>
					</div>
