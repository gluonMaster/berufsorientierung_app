/**
 * API Endpoint: POST /api/admin/reviews/approve
 *
 * Одобрение отзывов (только для администраторов)
 *
 * Функциональность:
 * - Одобрение отдельных отзывов по ID
 * - Массовое одобрение всех pending отзывов
...
export async function POST({ request, platform }: RequestEvent) {
	try {
		...
		const details = all_pending
			? `Approved all pending reviews (${updated} total)`
			: `Approved reviews: [${review_ids?.join(', ')}] (${updated} updated)`;
		...
		console.error('[POST /api/admin/reviews/approve] Error:', error);
